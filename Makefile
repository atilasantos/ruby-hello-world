...
REGISTRY:= atilarmao
DOCKER_REPO:= $(REGISTRY)/lovevery-hello
HELM_REPO:=  $(REGISTRY)/lovevery
TAG := 0.0.1
...
docker-build: ## Target specified to build and push the docker image for linux/amd64 arch, you can specify other architectures by `make build arch=linux/arm64`.
	@if [ -z "$(arch)" ]; then \
		docker buildx build --platform linux/amd64 -t $(DOCKER_REPO):$(TAG) . --push; \
	else \
		docker buildx build --platform linux/amd64,$(arch) -t $(DOCKER_REPO):$(TAG) . --push; \
	fi
...
docker-run: ## Target specified to run the builded application as a standalone container being port-forwareded to port 3000 of the host.
	docker container run -d -e SECRET_KEY_BASE=6dt7Q23Q1Z6P -p 3000:3000 $(DOCKER_REPO):$(TAG)

helm-package: ## Targe specified to package the helm chart and push it to dockerhub.
	helm package .helm/lovevery --version $(TAG)
	helm push "lovevery-$(TAG).tgz" "oci://docker.io/$(REGISTRY)"
	rm -f "lovevery-$(TAG).tgz"

helm-run: ## Target to install the helm release for the application.
	helm install hello-app oci://docker.io/$(HELM_REPO) --version $(TAG) -f ./.helm/lovevery/values.yaml --set "env[0].name=SECRET_KEY_BASE" --set "env[0].value=6dt7Q23Q1Z6P" -n lovevery --create-namespace
...
add-hosts: ## Target specified to add to /etc/hosts an entry where the app can be accessed via local.localhost in kubernetes deployments.
	...
	@IP=""
	@while [ -z "$$IP" ]; do \
		IP=$$(kubectl get ingress -n lovevery lovevery-hello -o jsonpath="{.status.loadBalancer.ingress[0].ip}"); \
		sleep 2; \
	done; \
